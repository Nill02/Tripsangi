...
export const intelligentBudgetPlanner = async ({ user, place, days, budget }) => {
  ...
  // ----------------------------------
  // Resolve `place` to a Location _id when a name is provided
  let locationId = place;
  try {
    const isObjectId = mongoose.Types.ObjectId.isValid(place);
    if (!isObjectId) {
      const locDoc = await Location.findOne({ name: { $regex: `^${place}$`, $options: 'i' } });
      if (locDoc) locationId = locDoc._id;
    }
  } catch (err) {
    // ignore and proceed with original place value
  }

  const hotels = await Hotel.find({ location: locationId });
  const transports = await Transport.find({ location: locationId });
